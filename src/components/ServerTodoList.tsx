...
interface ServerTodoListProps {
  page?: number;
  limit?: number;
}

export default async function ServerTodoList({ page = 1, limit = 10 }: ServerTodoListProps) {
  try {
    ...
    return (
      <div className="max-w-6xl mx-auto p-6">
        <div className="bg-gradient-to-r from-red-50 to-pink-50 border border-red-200 rounded-xl p-8 text-center">
          ...
          <h2 className="text-2xl font-bold text-red-800 mb-2">Server Error</h2>
          <p className="text-red-600">Failed to load todos from server. Please try again later.</p>
        </div>
      </div>
    );
  }
